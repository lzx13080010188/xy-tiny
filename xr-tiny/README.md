...
## 简介
基于mall-tiny修改的后端脚手架，新增文件服务器minio，还有日志aop的操作，后续准备整合flowable工作流，使开发者开发小项目，可直接上手
xy-tiny是一款基于SpringBoot+MyBatis-Plus的快速开发脚手架，拥有完整的权限管理功能，可对接Vue前端，开箱即用。

## 项目演示

xr-tiny项目可无缝对接`mall-admin-web`前端项目，秒变权限管理系统。前端项目地址：https://github.com/macrozheng/mall-admin-web
...
简化依赖服务，只需安装最常用的MySql和Redis服务即可，数据库中需要导入`xr_tiny.sql`脚本。
...
├── domain -- 共用封装对象
├── generator -- MyBatis-Plus代码生成器，实体转mysql数据库生成器
├── modules -- 存放业务代码的基础包
|   └── ums -- 权限管理模块业务代码
|       ├── controller -- 该模块相关接口
|       ├── dto -- 该模块数据传输封装对象
...
直接运行启动类`XrTinyApplication`的`main`函数即可。
...
<mapper namespace="UmsMenuMapper">
    ...
    <select id="getMenuList" resultType="com.xr.tiny.modules.ums.model.UmsMenu">
        SELECT
            m.id id,
